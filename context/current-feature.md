...
- [ ] 2. feat(data): stored runs record their endpoint set (`ps80` / `alx117` / none) at start and on identity resolve; ALX-117 step notes from S15; `getDossier` with partial filtering and the novel rule; dossier route; unit tests incl. another user's run → 404 and partial = 3 precedent rows.
